    ...
    def render(self):
                ...
                if st.button("Run Code", key="run_code_button"):
                    if config.ENABLE_CODE_EXECUTION:
                        with st.spinner("Running code..."):
                            ...
                            utils.save_to_session_state("execution_results", {
                                "success": success,
                                "output": output,
                                "error_msg": error_msg
                            })
                    else:
                        st.warning("Code execution is disabled for security reasons. You can enable it in the config.py file.")
        ...
        if generated_code and st.button("Explain This Code", key="explain_code_button"):
            with st.spinner("Generating explanation..."):
                prompt = f"Explain the following code in detail, focusing on the ML/DL concepts and implementation details:\n\n{generated_code}"
        ...
        if st.button("View Coding History", key="view_coding_history_button"):
            ...
            if coding_history:
                ...
                for i, session in enumerate(coding_history[:5]):  # Show only the 5 most recent sessions
                    with st.expander(f"Session {i+1}: {session['topic']} ({session['timestamp']})"):
                        ...
                        if st.button(f"Load Session {i+1}", key=f"load_session_{session['id']}"):
                            session_details = self.history_manager.get_session_details(session['id'])
                            if session_details:
                                utils.save_to_session_state("generated_code", session_details['code'])
